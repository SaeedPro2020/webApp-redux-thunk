...
export default function ShowPosts(props: propsParm): JSX.Element {
  ...
  const userComments = (postId: number) => {
    setBtnComment(true);
    setpostId(postId);
    dispatch(GetComments(postId) as unknown as AnyAction);
    ...
    <div className="rootContainer">
      ...
      <ul>
        {props.onData?.posts?.map((postData: PostModel) => (
          <div key={postData.id}>
            <li className="postLists">{"Title: " + postData?.title}</li>
            <li className="postLists">{"Body" + postData?.body}</li>

            <button
              className="Buttons"
              onClick={() => userComments(postData.id)}
            >
